  ...
  db.collection("jobOfferPost")
    ...
    .then((snapshot) => {
      snapshot.forEach((doc) => {
        ...
        if (
          (payboost == "true" && boostStartDate < today) ||
          boostStartDate == today
        ) {
          payboosted = true;
  ...
  function sortAndFilter() {
    ...
    if (sort == "latestOrder") {
      db.collection("jobOfferPost")
        ...
        .then((snapshot) => {
          ...
          snapshot.forEach((doc) => {
            ...
            if (
              (periodList[period] == 0) |
              (periodList[period] == 1 && day <= 1) |
              (periodList[period] == 2 && day > 1 && day <= 7) |
              (periodList[period] == 3 && day > 7 && day <= 30) |
              (periodList[period] == 4 && day > 30)
            ) {
                  ...
                  if ((gender == "no") | (gender == doc.data().gender)) {
                    if (postendDate.valueOf >= presentDate.valueOf) {
                      ...
                      if (
                        ...
                        payboosted = true;
                        postPay = postPay + postPay * (0.01 * increaseRate);
                      }
      ...
      <label class='postEnd'><b>마감일</b> ${postEnd}</label>
                      ...
                      if (payboosted) {
                        document.getElementById(doc.id).style.border =
                          "2px solid red";
                      }
                    }
                  }
