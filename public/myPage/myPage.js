        ...
        snapshot.forEach((doc) => {
            ...
            if (id == writer_id) {
                var title = doc
                    ...
                    .title;
                var post = `<div id='${doc
                    .id}' class='object writePost jobOfferPost'><b>구인</b> ${title}</div>`
                    $("#writePostList")
                    .append(post);
                        ...
                        db
                            ...
                            .then((doc) => {
                                if (end.valueOf() < present.valueOf()) {
                                    var applyerpost = `<div>
          <div id='${post_id}' class='object applicant jobOfferPost'><b id='${doc
                                        .data()
                                        .email}'>${doc
                                        .data()
                                        .name}</b> ${title}</div>
                        ...
                        for (i in doc.data().applicantList) {
                            ...
                            db
                                ...
                                .then((doc) => {
                                    ...
                                    var applyerpost = `<div>
              <div id='${post_id}' class='object applicant jobOfferPost'><b id='${applicant}'>${doc
                                        .data()
                                        .name}</b> ${title}</div>
              <div id="applicantBtn">
                          <button id="profileBtn" class="Btn">프로필보기</button>
        ...
        snapshot.forEach((doc) => {
            ...
            if (id == writer_id) {
                var title = doc
                    ...
                    .title;
                var post = `<div id='${doc
                    .id}' class='object writePost jobSearchPost'><b>구직</b> ${title}</div>`
                    $("#writePostList")
                    .append(post);
            }
        })
    ...
    .then((snapshot) => {
        snapshot.forEach((doc) => {
            var title = doc
                ...
                .title;
            var post = `<div id='${doc
                .id}' class='object applyPost jobOfferPost'><b>구인</b> ${title}</div>`
                $("#applyPostList")
                .append(post);
        });
    });
